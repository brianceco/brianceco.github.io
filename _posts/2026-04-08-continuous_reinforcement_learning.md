...
# **1. What is (discrete) RL?** 
Let's begin by quickly recalling RL theory in discrete time and space, fixing notation and outlining what results we actually want to extend to the continuous setting. 

Let's consider a (time-homogeneous) Markov Decision Process (MDP) $$\{X_t\} _{t =0}^{\infty}$$ with finite state space $\mathcal{X}$ and finite action space $\mathcal{A}$. The dynamics of $X_t$ are governed by a transition kernel 

$$p(x^\prime|x,a)\coloneqq \mathbf{P}(X_{t+1}^{a}=x^\prime|X_{t}=x,A_t=a)$$ 

from $\mathcal{X} \times \mathcal{A}$ to $\mathcal{X}$, where $$\{A_{t}\}_{t=0}^{\infty}$$ is the action process following an admissible policy $$A_t \sim \pi(da\mid X^{A}_t) \in \euscr{P}(\mathcal{A})$$. We are interested in the episodic setting, in which our environment evolves over some finite time horizon $$T \in \mathbf{N}$$. The goal is to identify an admissible policy $$\pi^{\ast}=\pi^{\phi^{\ast}}$$ from some parametric family $$\phi   \in \Phi \subset \mathbf{R}^{N_\phi}$$ which is *optimal* from the point of view that it maximizes the value function 

$$
J(x;\pi) \coloneqq \E_{x}\left[ \sum\limits_{t=0}^{T}\beta ^{t} r(X^{A}_t,A_t) \right] 
$$ 

defined as the cumulative discounted expected reward, with discount rate $\beta  \in (0,1)$ and reward function $r\colon \mathcal{X} \times \mathcal{A}\to \euscr{P}(\mathbf{R})$.
...
$$
Q(x_0,a_0;\phi )=\E[r(x_0,a_0)] + \beta \sum\limits_{x_1 \in \mathcal{X}}^{} p(x_1|x_0,a_0)J(x_1;\phi )
$$ 

where implicitly we note $J(x_1;\phi)$ is a summation up to $T-1$. Substituting this into the previous equation yields 
...
$$\begin{align*}
\frac{\partial J}{\partial \phi } (x_0;\phi ) &=   \sum\limits_{x \in \mathcal{X}}^{} \E_{x_0}\left[ \sum\limits_{t=0}^{T} \beta ^{t}\bm{1}_{\{X^{\phi }_t=x\} } \right] \ell (x)\\
&= \E_{x_0} \left[\sum\limits_{t=0}^{T} \beta ^{t}  \sum\limits_{x \in \mathcal{X}}^{}\bm{1}_{\{X^{\phi }_t=x\} }\ell (X^{\phi }_t)  \right]  \\
&= \E_{x_0} \left[ \sum\limits_{t=0}^{T} \beta ^{t} \ell (X^{\phi }_t) \right] \\
&= \E_{x_0} \left[ \sum\limits_{t=0}^{T} \beta ^{t} \sum\limits_{a \in \mathcal{A}}^{} \pi^{\phi }(a|X^{\phi }_t)\frac{\partial \log \pi^{\phi }}{\partial \phi } (a|X^{\phi }_t) Q(X^{\phi }_t,a;\phi ) \right] \\
&=  \E_{x_0}\left[ \sum\limits_{t=0}^{T} \beta ^{t} \E\left[ \frac{\partial \log \pi^{\phi }}{\partial \phi } (A^{\phi }_t|X^{\phi }_t)Q(X^{\phi}_t,A^{\phi }_t;\phi )|X^{\phi }_t \right]  \right] \\
&=  \E_{x_0}\left[ \sum\limits_{t=0}^{T} \beta ^{t} \frac{\partial \log \pi^{\phi }}{\partial \phi } (A^{\phi }_t|X^{\phi }_t)Q(X^{\phi}_t,A^{\phi }_t;\phi ) \right] \\
&=  \E_{x_0}\left[ \sum\limits_{t=0}^{T} \beta ^{t} \frac{\partial \log \pi^{\phi }}{\partial \phi } (A^{\phi }_t|X^{\phi }_t)\left(Q(X^{\phi}_t,A^{\phi }_t;\phi ) -B(X^{\phi })\right)\right] 
\end{align*}$$
...
## **2.1. Exploration: relaxed stochastic control**
Stochastic control (SC) and reinforcement learning (RL) are both methods for optimizing sequential decision making under uncertainty. The idea is that an agent is able to interact with their environment (state) via a series of actions (controls). Where SC and RL diverge is that the former is *model-based*, i.e. it presumes one has perfect knowledge of the dynamics by which the environment (stochastically) evolves, and consequently seeks to obtain existence/uniqueness results for an optimal *feedback* control defined in terms of a deterministic policy of the state. In contrast, RL begins from a starting point of incomplete information, where an agent only learns about the (unknown) dynamics of their environment via their interaction with it, generating realization(s) from which the agent can learn an (approximately) optimal policy. In order to achieve this, the agent chooses actions both to *optimize their reward* (as in SC) and to *explore their environment*. It is this latter condition which necessitates actions being sampled *randomly* according to a policy (distribution), independently of the randomness of the environment. The correct way to include randomness in control into the SC setting is via the theory of relaxed stochastic control. 

Let's begin with classical the classical setup. We fix the canonical path space $\Omega=C([0,T], \mathbf{R})$ equipped with the Borel $\sigma$-algebra $\mathcal{B}$ induced by the topology of uniform convergence, and the Wiener measure $\mathbf{P}_W$. Let $\mathcal{F}^{W}_t$ denote the associated filtration generated by $W _t$. Consider a state process $X_t^{a}$ which evolves via an Ito diffusion 
...
**Assumptions 1.** 
1. The functions $b,\sigma, r,h$ are continuous in their respective arguments.
...
\begin{equation}\label{eqn:polygrowth}
        \E^{\mathbf{P}^{W}}[\sup _{0 \leq t \leq T}|\overline{X}^{\pi}_t|^{\mu }]  \leq C(1+|x| ^{\mu }).
\end{equation}
$$ 
...
3. Again using the martingale orthogonality condition with test process $\xi_t=\frac{\partial J^{\theta }}{\partial \theta } (t,X^{\pi}_t)$, but now choose $\theta \in \Theta $ via minimization of the generalized methods of moments (GMM) loss functional
...
## **2.3. Improvement: PG (PE in disguise)**
...
**Assumptions 2.**
1. For all $\phi \in \Phi$, $\int_{\mathcal{A}}^{} \mid\widetilde{r}(t,x,a;\phi )\mid \pi^{\phi }(a\mid t,x)da$ is of polynomial growth in $x$ uniformly in $t$.
2. For all $\phi \in \Phi$, $\int_{\mathcal{A}}^{} \left\lvert\frac{\partial }{\partial \phi } \log \pi^{\phi }(a\mid t,x)\right\rvert^2 \pi^{\phi }(a \mid t,x) \,da$ is continuous in $(t,x)$.
...
<details>
<summary>Proof.</summary>
Define the exit times $\tau _{n}\coloneqq \inf \{t:|X^{\phi }_{t}| \geq n \} .$ Applying Ito's lemma to $J(s,X^{\phi }_s)$, we have 
...
We claim the Ito integral is a true $(\mathcal{F}_t, \mathbf{P})$-martingale and hence its expectation vanishes. For $s \in [t, T \wedge \tau _n]$, we have the bound 
...
where $C=C(n,\phi,T)$. Thus, taking expectations above and comparing terms, we see

$$
\begin{align*}
&\E^{\mathbf{P}}_{t,x}\left[\int_{t}^{T \wedge \tau _n} e^{-\beta (s-t)} \left(\frac{\partial }{\partial \phi } \log \pi^{\phi }(A^{\pi}_s|s,X^{\phi }_s)e^{\beta s}dM_s^{\phi} + \gamma q(s,X^{\phi }_s,A^{\phi}_s)ds\right)\right]\\
&= \E^{\mathbf{P}}_{t,x}  \left[ \int_{t}^{T \wedge \tau _n} e^{-\beta (s-t)}\widetilde{r}(s,X^{\phi }_s,A^{\phi }_s;\phi )\,ds \right] 
.\end{align*} 
$$

Finally, it remains justifying applying dominated convergence to take the limit $n \to \infty$. We have 

$$
\begin{align*}
        &\E^{\mathbf{P}^{W}}_{t,x}\left[\int_{t}^{T} e^{-\beta (s-t)}\int_{\mathcal{A}}^{} |\widetilde{r}(s,X^{\phi }_s,a;\phi )|\pi(a|s,X^{\phi }_s)\,dadt\right] \\
        & \leq C\E^{\mathbf{P}^{W}}\left[1+\sup _{t \leq s \leq T}|X^{\phi }_s|^{\mu }|\right] \tag{Assumption 2}\\
        & \leq C (1+|x|^{\mu }) \tag{Equation \ref{eqn:polygrowth}}
\end{align*}
$$
</details>
...
$$
\begin{align*}
        0&=    \int_{\mathcal{A}}^{} \widetilde{r}(t,X^{\ast}_s,a;\phi ^{\ast})\pi^{\phi ^{\ast}}(a|t,x)da
.\end{align*} 
$$

From this, together with the fact that we showed the local-martingale $N^{a}_t$ is a true martingale, we obtain the following in exactly the same way as Theorem 1.
...
Finally, as an application of the continuous RL we have developed, let's apply the AC algorithm to a toy mean-variance portfolio optimization considered in {% cite wang2020mvo %}. Consider a stock price $S_t$ which follows a GBM

$$
        \frac{dS_t}{S_t} = \mu  dt + \sigma dW_t
.$$

We assume a risk-free rate $r>0$, a nondegenerate volatility $\sigma>0$, and an initial portfolio balance $x_0$. Letting $A_t$ denote the discounted dollar holdings in the stock at time $t$, the associated discounted PnL of the portfolio evolves via the dynamics 

$$
d X^{A}_t = A_t[(\mu - r )dt + \sigma dW_t] = a_t \frac{d(e^{-rt}S_t)}{e^{-rt}S_t}
.$$

Our goal is to optimize the terminal mean-variance criterion 

$$
J^{A}(x_0)=\inf_{A} \E[(X^{A}_T-w)^2] - (w-z)^2
$$ 
...
$$
        J^{\theta }(t,x;w) = (x-w)^2 e^{-\theta _3(T-t)}+\theta _2(t^2-T^2) + \theta _1 (t-T)-(w-z)^2
$$ 

and Gaussian policies 

$$
\pi^{\phi }(da|t,x;w)= \mathcal{N}(da|-\phi _1(x-w),e^{\phi _2+\phi _3(T-t)}).
.$$ 
...
We set the discount rate $\beta =0$, and the temperature parameter $\gamma=0.1$. For PE, we use the TD(0) algorithm, which requires the critic gradients 

$$
\frac{\partial J^{\theta }}{\partial \theta _1} (t,x;w)=t-T, \hspace{10pt} \frac{\partial J^{\theta }}{\partial \theta _2} (t,x;w)= t^2-T^2, \hspace{10pt} \frac{\partial J^{\theta }}{\partial \theta _3} (t,x;w) = (x-w)^2 e^{-\theta _3(T-t)}(t-T).
$$ 

For PG, the gradient of the log-likelihood is given by 
...
In addition to the critic and actor parameters, we need to learn the Lagrange multiplier $w$ associated to our target discounted mean wealth $z$. We do this via stochastic approximation 

$$
        w \leftarrow w - \alpha_w \frac{1}{m} \sum\limits_{i=j-m+1}^{j} X^{(i)}_T.
$$
...
We consider $\Delta t = \frac{1}{252}$ daily return data. For the offline algorithm, we simulate 20 years of training data, which we randomly sub-sample in batches of 128 1-year blocks, for a total of $N=2000$ episodes. We set the batch size for updating the Lagrange multiplier to $m=10$ episodes. For the online algorithm, we consider 100 years of training data, and we update our actor and critic after each time step, and our Lagrange multiplier after each episode/year ($m=1$). We repeat the experiment with 100 samples, and report the mean and standard errors of the mean, variance, and Sharpe ratio of terminal discounted wealth $X_T$.
...
{% bibliography --cited --template bib_post --group_by none %}
</div>
